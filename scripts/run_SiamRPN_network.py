...
def tracker_eval_record_data(scorenet, net, x_crop, target_pos, target_sz, window, scale_z, p, im, next_mask,conf_mask,gtbbox):
    ...
    print(y)
    target_pos = (np.array([temp_result[chosen_id,0] + (temp_result[chosen_id,2]/2), 
        temp_result[chosen_id,1] + (temp_result[chosen_id,3]/2)]))
...
def SiamRPN_init(im, target_pos, target_sz, net):
    ...
    return state
def SiamRPN_track_bbox(scorenet, state, im, next_mask, conf_mask, gtbbox):
    p = state['p']
    net = state['net']
    ...
    target_pos, target_sz, score, alternative = tracker_eval_record_data(scorenet, net, x_crop.cuda(), target_pos, target_sz * scale_z, window, scale_z, p, im, next_mask,conf_mask,gtbbox)
