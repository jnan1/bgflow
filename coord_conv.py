...
def concatenate_coords(inp):
    ...
    coords = torch.from_numpy(coords) # convert to torch tensor
    coords = coords.unsqueeze(0).expand(batch_size, -1, -1, -1)
    coords = Variable(coords).cuda().float() 
...
def base_conv(batchNorm, in_planes, out_planes, kernel_size, stride, conv_layer):
    if batchNorm:
        return nn.Sequential(
            conv_layer(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=(kernel_size-1)//2, bias=False),
            nn.BatchNorm2d(out_planes),
            nn.LeakyReLU(0.1,inplace=True)
        )
    else:
        return nn.Sequential(
            conv_layer(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=(kernel_size-1)//2, bias=True),
            nn.LeakyReLU(0.1,inplace=True)
        )


def conv(batchNorm, in_planes, out_planes, kernel_size=3, stride=1):
    return base_conv(batchNorm, in_planes, out_planes, kernel_size, stride, nn.Conv2d)


def coord_conv(batchNorm, in_planes, out_planes, kernel_size=3, stride=1):
    return base_conv(batchNorm, in_planes, out_planes, kernel_size, stride, CoordConv2d)
